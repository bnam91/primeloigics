...
def 반품처리_창():
    ...
    반품_창 = tk.Toplevel()
    반품_창.title("반품 정보 입력")
    반품_창.geometry("750x650")  # 창 크기 조정
    
    # 상단 버튼 프레임 - 심플한 디자인으로 변경
    버튼_프레임 = tk.Frame(반품_창)
...
''')
    ...
    def 분석_실행(텍스트_영역, 쇼핑몰_변수):
        ...
        def 백그라운드_분석():
            ...
            try:
                ...
                반품_창.after(0, lambda: 분석_결과_표시(분석결과, 탭_데이터))
            except Exception as e:
                # 오류 발생 시 UI 업데이트
                반품_창.after(0, lambda: 분석_오류_표시(str(e), 탭_데이터))
    ...
    # 디버깅용 레이블 추가
    상태_레이블 = tk.Label(반품_창, text="GUI가 정상적으로 로드되었습니다", fg="green")
